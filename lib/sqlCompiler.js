...
function keys(argSpec, sep = ",", wrapStr='%') {//{{{
    ...
    const me = this;
    const keyParser = hlp.softBind(literal, me);
...
function values(argSpec, sep=",", wrapStr='%') {//{{{
    if (typeof argSpec != "object") typeSpec = [argSpec];
    const me = this;
    const valParser = hlp.softBind(arg, me);
    sep = sep.replace(D.re_spacing, ' ');
    if (! wrapStr.match('%')) wrapStr += ' %';
...
function entries(argSpec, sep=",", wrapStr='%', formatStr='%1 = %2') {//{{{
    if (typeof argSpec != "object") typeSpec = [argSpec];
    const me = this;
    const keyParser = hlp.softBind(literal, me);
    const valParser = hlp.softBind(arg, me);
    ...
    const contents = Object.entries(argSpec)
        .map(function(pair) {
            ...
            ;
            return formatStr
                .replace(/(?<!\\)%1(?!\d)/g, keyParser(key).render())
                .replace(/(?<!\\)%2(?!\d)/g, valParser(value).render())
            ;
        })
    ;
    return new interpolation (
