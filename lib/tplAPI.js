...
module.exports = {
    ...
    sql(engName) {//{{{
        ...
        const cache = me[_priv_].sqlCache; 
            // Cache is not available when this method is called from $.include()
        if (
            cache
            && cache[eng.name] !== undefined
        ) return me[_priv_].sqlCache[eng.name];
        ...
        if (cache) cache[eng.name] = outSql;
