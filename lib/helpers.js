...
function deepCopy(master) {//{{{
    if (master instanceof Array) {
        return Array.from(master, deepCopy);
...
module.exports = {
    ...
    parseJSON: JSON.parse.bind(JSON), // Do things well even unecessary.
    instantiate(target, constructor) {//{{{
        const p1 = target && Object.getPrototypeOf(target);
        const p2 = Object.getPrototypeOf(new constructor(''));
        return (
            p1 === p2 ? target
